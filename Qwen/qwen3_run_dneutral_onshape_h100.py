...
def summarize_feature(feat: Dict[str, Any], idx: int, entity_sample: int = 6) -> str:
    ...
    if str(t).lower() == "chamfer":
        d1 = (feat.get("chamfer_d1_mm") or feat.get("d1_mm")
              or feat.get("distance1_mm") or feat.get("distance_mm"))
        d2 = (feat.get("chamfer_d2_mm") or feat.get("d2_mm")
              or feat.get("distance2_mm"))
    ...
    if str(t).lower() in ["pattern", "linearpattern", "circularpattern"]:
        ...
        sp = feat.get("spacing_mm") or feat.get("pitch_mm")
        if isinstance(sp, (int, float)):
            parts.append(f"spacing_mm={sp}")
        ang = feat.get("angle_deg")
        if isinstance(ang, (int, float)):
            parts.append(f"angle_deg={ang}")
    ...
    if str(t).lower() in ["hole", "drill", "holefeature"]:
        ...
        if isinstance(depth, (int, float)):
            parts.append(f"depth_mm={depth}")

    if str(t).lower() == "shell":
        th = feat.get("thickness_mm")
        if isinstance(th, (int, float)):
            parts.append(f"thickness_mm={th}")
...
def ir_to_text_summary(ir: Dict[str, Any], max_features: int = 32) -> str:
    ...
    if nums:
        nums_str = ", ".join(
            [str(int(x)) if abs(x - round(x)) < 1e-6 else f"{x:.6f}".rstrip("0").rstrip(".") for x in nums[:80]]
        )
...
def clean_output(text: str) -> str:
    ...
    if not out:
        out = (
            "Create the shape using the sketches and features shown in the model, "
            "then apply the same extrusions, cuts, and edge treatments to match the overall form."
        )
    return out
...
def main() -> None:
    ...
    if args.neutral_json:
        ir_path = Path(args.neutral_json).resolve()
        if not ir_path.exists():
            raise SystemExit(f"[ERR] neutral_json not found: {ir_path}")
        if not args.out_txt:
            raise SystemExit("[ERR] --out_txt is required with --neutral_json")
    ...
    if not args.neutral_root or not args.out_root:
        raise SystemExit("[ERR] Dir mode requires --neutral_root and --out_root (or use --neutral_json + --out_txt).")
